...
- [x] 多輪估點功能
- [x] 極端值分析（高亮顯示，原因由團隊口頭討論）
- [x] Issue 管理
  - [x] Host 可以建立多個 Issue
  - [x] Host 可以在開始估點前選擇要估點哪一個 Issue
  - [x] 如果列表中沒有 Issue 時，點擊「開始估點」會自動建立一個未命名的 Issue（依照 Host 的語系命名）
  - [x] Issue 重新命名功能
  - [x] 估點進行中時可以建立新 Issue（提升會議效率，避免等待時間浪費）
- [x] QR Code 掃描（Client 端）
- [x] 統計圖表（視覺化）
- [x] Host 最終決定權（Host 可以選擇最終的估點值）
- [x] 多國語系擴展：簡體中文（zh-CN）、日文（ja）
- [x] 使用者名稱重複檢查（Client 端加入時檢查是否與其他參與者或 Host 重複）
- [x] 語言切換改為 Dropdown 選單
- [x] Client 端顯示當前估點的 Issue 資訊（標題和描述）

### 第四階段：優化與增強

- [x] 動畫效果優化
  - [x] 簡易模式下，個人卡片選擇後的互動效果
    - [x] **滑鼠優先邏輯**：如果偵測到滑鼠輸入，優先使用滑鼠 hover 效果，即使裝置支援陀螺儀也不使用陀螺儀效果
    - [x] 支援陀螺儀的裝置（無滑鼠輸入時）：使用陀螺儀讓卡片隨裝置傾斜而傾斜（例如手機往左下傾斜，卡片也會向左下傾斜）
    - [x] 不支援陀螺儀或偵測到滑鼠的裝置：使用滑鼠 hover 效果調整卡片角度（例如滑鼠放到卡片左下角，卡片會向左下傾斜）
    - [x] 翻牌動畫與傾斜效果分離：翻牌時保留平滑的翻轉動畫，傾斜時提供即時響應
- [ ] 效能優化
- [ ] 離線支援（PWA）
- [ ] 自動重連機制
- [ ] 自訂信號伺服器功能
- [ ] 動態更新頁面標題（`<title>`）：不同頁面應顯示對應的標題
